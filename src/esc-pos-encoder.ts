...
export default class EscPosEncoder {
  ...
  setPinterType(type: PrinterWidthEnum): EscPosEncoder {
    if (type === PrinterWidthEnum._58) {
      ...
      this._printerParam = this._76printerParam;
  ...
  printFrontDeskDishs({
    ...
    dishes.forEach((dish, index) => {
      ...
      if (showUnitPrice) {
        ...
        const fixedWidthStrArr = this.splitByWidth(dish.name, this.singleCharLengthPerLine - countAndPriceLength - 2);
      ...
      if (specificationInNewLine) {
        dish.specifications?.forEach((str) => {
          if (str) {
            this.line('  * ' + str + ' *');
          }
        });
  ...
  printChefDishs({
    ...
    dishes.forEach((dish, index) => {
      ...
      if (countFront) {
        ...
        const fixedWidthStrArr = this.splitByWidth(dish.name, this.singleCharLengthPerLine - countAndPriceLength);
        fixedWidthStrArr.forEach((str, index) => {
          if (index === 0) {
  ...
  size(value: number): EscPosEncoder {
    ...
    this._queue([0x1b, 0x4d, 0x00]);
    this._queue([0x1d, 0x21, realSize]);
   ...
   */
  cut(): EscPosEncoder {
    this._queue([0x1d, 0x56, 0x41, 0x00]);
  ...
  openCashBox(): EscPosEncoder {
    this._queue([0x1b, 0x70, 0x00, 0x3c, 0xff]);
  ...
  cutPartial(): EscPosEncoder {
    this._queue([0x1d, 0x56, 0x42, 0x00]);
